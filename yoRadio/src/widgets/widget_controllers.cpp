...
MessageQ_Controller::MessageQ_Controller(const char* label, const char* name_space, MessagePool& pool, std::shared_ptr<AGFX_TextScroller> unit, int32_t qid, size_t qlen) :
  EmbUIUnit(label, name_space), _pool(pool), _unit(unit), qid(qid), _max_q_len(qlen) {
  // create timer
...
void MessageQ_Controller::_consume_msg(){
  LOGV(T_MQCtrl, printf, "_consume_msg gid:%u, q size:%u\n", qid, _pool._msg_list.size());

  // try to evict as many msgs from a Q as possible
  while (auto m = _pool.evictMessage(qid)){
    // a short cut - pick the message if currently nothing is scrolled at all
    if (!_current_msg){
      _current_msg.reset(m);
    ...
    if (m->id){
      ...
      if (_current_msg && _current_msg->id == m->id){
        ...
        auto lookup = std::find_if(_mqueue.begin(), _mqueue.end(), [id](const TextMessage_pt &itm){ return id == itm->id; });
    ...
    if (_mqueue.size() < _max_q_len){
      ...
      _mqueue.emplace_back(m);
    }
    else
      delete m;    // if queue is full, then discard new message
  }
}
...
void MessageQ_Controller::_evaluate_queue(){
  ...
  auto i = std::find_if(_mqueue.begin(), _mqueue.end(), [](const TextMessage_pt& m){ return millis() - m->last_displayed >= m->interval;});
